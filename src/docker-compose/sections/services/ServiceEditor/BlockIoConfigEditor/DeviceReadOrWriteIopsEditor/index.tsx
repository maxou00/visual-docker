import {
  ...
  import { DeviceIops, DeviceRate } from "../../../../../types";
  
  interface Props {
    value?: DeviceIops[];
    onChange: (update: DeviceIops[]) => any;
  }
  ...
  const initial = { id: nanoid(), path: "", rate: 120 };
    ...
    return (
      <HStack w="full" alignItems="center" spacing={2}>
        ...
        <Input
          ...
          value={entry.rate}
          type="number"
          onChange={({ target }) => onChange({ ...entry, rate: target.valueAsNumber })}
        />
  ...
  export default function DeviceReadOrWriteIopsInput({
    value,
    onChange,
  }: {
    value: any;
    ...
    const onAppendKey = useCallback(() => {
      ...
      cpy.push({ id: nanoid(), path: "", rate: 120 });
      onChange(cpy);
      setKeys(cpy);
    }, [keys, onChange]);
